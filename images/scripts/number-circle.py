...
dwg = svgwrite.Drawing('../number-circle.svg', profile='tiny', size=(f'{width}', f'{height}'),viewBox=(f'0 0 {width} {height}'))
...
label_group = svgwrite.container.Group(style='font-size:1.5rem;font-family:sans-serif;fill:black;dominant-baseline:central;text-anchor:middle;text-rendering:geometricPrecision')
...
for segment_number in range(0, segment_count):
    ...
    lx = 1.1 * (radius+marker_length/2.0) * math.cos(rad) + cx
    ly = 1.1 * (radius+marker_length/2.0) * math.sin(rad) + cy

#    label_line = dwg.line((ex, ey), (lx, ly), stroke='red', stroke_width=sw)
#    shapes.add(label_line)

    label = dwg.text(str(segment_number), insert=(lx, ly))
